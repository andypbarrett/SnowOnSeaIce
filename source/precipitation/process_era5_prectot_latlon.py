...
def process_one_year(year, region='globe', verbose=False):
    ...
    dsMon = ds.resample(time='MS').mean(dim='time')
...
def main():
    ...
    if region == 'arctic':
        fileout = os.path.join(dirpath,'era5.precip_rate.arctic.csv')
    else:
        fileout = os.path.join(dirpath,'era5.precip_rate.global_mean.csv')
