...
def main():
    ...
    cfsr_prectot = get_data(fili['CFSR'], 'wetdayAve')
    erai_prectot = get_data(fili['ERAI'], 'wetdayAve')
    mer2_prectot = get_data(fili['MERRA2'], 'wetdayAve')
    merr_prectot = get_data(fili['MERRA'], 'wetdayAve')
    ja55_prectot = get_data(fili['JRA55'], 'wetdayAve')
    era5_prectot = get_data(fili['ERA5'], 'wetdayAve')
    ...
    ax.set_title('August to April Mean Wetday Precipitation', fontsize=20)
    ...
    fig.savefig('reanalysis_arctic_mean_wetday_mean_Nh50km.png')
